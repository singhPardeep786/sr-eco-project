...
const Features = () => {
  ...
  // Notify Lenis that content size changed
  useEffect(() => {
    // @ts-ignore
  ...
  // Scroll to the last row after loading more
  useLayoutEffect(() => {
    if (shouldScroll && lastRowRef.current) {
      // Run after layout paints so the new height is measurable
      requestAnimationFrame(() => {
        const top = lastRowRef.current!.getBoundingClientRect().top + window.scrollY - 100;
        // @ts-ignore
        const lenis = window?.lenis;
        if (lenis && typeof lenis.scrollTo === "function") {
          lenis.scrollTo(top, { duration: 1, offset: 0 });
        } else {
    ...
    <>
      <section className='wrapper my-8'>
        ...
        <div className='flex flex-col items-center justify-center gap-3 w-full'>
          {rows.map((row, rowIdx) => {
            // Attach ref to the last row only when loading more
            const isLastRow = rowIdx === rows.length - 1;
            return (
              <div
                className="md:flex flex-row items-center justify-center gap-3 w-full"
                key={rowIdx}
              ...
              >
                {row.map((box, idx) => {
                  ...
                  return (
                    <div
                      className={`${box.className} w-full md:w-[30%] mt-1 ${isNew ? 'fade-in-up' : ''}`}
                      key={idx}
                    >
        ...
        {visibleCount < 30 && (
          <div className="flex justify-center mt-3">
            <button
              className="px-6 py-3 w-[75%] bg-[var(--blue)] text-white rounded-lg font-semibold border border-[var(--blue)] hover:bg-transparent hover:text-[var(--blue)] transition-all duration-500 focus:outline-none cursor-pointer"
              style={{
                borderWidth: '1px',
                borderStyle: 'solid',
